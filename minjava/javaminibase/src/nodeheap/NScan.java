	   ...
	   IOException
{
  ...
  /** there is a datapage record on the first directory page: */
	...
	try {
        rectuple = dirpage.getNode(datapageRid);
	}  
 ...
 */
private boolean mvNext(NID rid) 
  throws InvalidTupleSizeException,
	   IOException
{
  NID nextnid;
  ...
  if (datapage == null)
  	...
  	nextnid = datapage.nextNode(rid);
	
	if( nextnid != null ){
	  userrid.pageNo.pid = nextnid.pageNo.pid;
	  userrid.slotNo = nextnid.slotNo;
	  return true;
	} else {
