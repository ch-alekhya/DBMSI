	   ...
	   IOException
  {
	...
	private boolean firstDataPage() throws InvalidTupleSizeException, IOException {
		...
		if (datapageEid != null) {
			...
			try {
				rectuple = dirpage.getEdge(datapageEid);
			}
			...
			dpinfo = new EdgeDataPageInfo(rectuple);
			...
			if (nextDirPageId.pid != INVALID_PAGE) {
				...
				if (datapageEid != null) {
					...
					if (rectuple.getLength() != EdgeDataPageInfo.size)
						return false;
	...
	private boolean mvNext(EID eid) throws InvalidTupleSizeException, IOException {
		...
		nexteid = datapage.nextEdge(eid);

		if (nexteid != null) {
			usereid.pageNo.pid = nexteid.pageNo.pid;
			usereid.slotNo = nexteid.slotNo;
			return true;
		} else {
